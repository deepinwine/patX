...
| **审查信息同步 (Dossier Sync)** | CN 官方发文后台巡检：USPTO Global Dossier → EPO Register → CNIPA 三层数据源自动降级，跨来源事件去重（event_key），自动识别第一次/第N次审查意见、驳回、授权、补正等官方事件并按保护规则入库；桌面通知提醒 |
| 年费 | 从真实授权专利生成（中国官费标准）；未配置规则的地区显示 "Rule not configured"，不再显示假数据 |
| 期限规则 | **期限计算底座**（细则4/5条：对应日、月末收敛、节假日顺延）+ 完整 CN 规则集 `data/deadline_rules/CN_deadline_rules.json`（62 条：优先权/OA/复审/无效/PCT国家阶段/年费/补偿/诉讼等，含起算点、可延期、可恢复、后果、生效版本）；发明 OA 分档：一通 4 个月、后续 2 个月；2026-09-01 起新版优先审查单独分支（发文日起 1 个月/15 日）；数据库规则表支持增删改，人工期限（deadline_source=manual）不被覆盖 |

## 审查意见网页同步与后台提醒（CN 案件）

首期只检查 CN 案件。数据源按固定顺序自动降级：**USPTO Global Dossier（公开 JSON API，无 Key，2026-09 实测打通）→ EPO Patent Register（公共页面，无 Key；站点有人机验证，多数情况自动降级）→ CNIPA（需登录）**。前两层是五局共享数据，更新通常滞后几天到几周——适合巡检提醒，不是实时。

- **后台静默巡检**：启动约 30 秒后首次检查，之后每 30 分钟调度一次；到期的案件（`next_dossier_check_at`）按设置的频率（每天 / 每 3 天 / 每 7 天，菜单 `审查信息同步 → 审查提醒设置`）逐件查询。无变化时完全静默；发现新官方事件/日期冲突/CNIPA 需要登录时弹桌面通知
- **只处理 ORIGINAL 官方发文**（TRANSLATED 机翻副本与申请人提交文件直接丢弃），且只入库元数据，不下载正文
- **跨来源去重**：同一官方事件在 USPTO/EPO/CNIPA 看到都收敛为一条 `prosecution_documents` 记录（event_key 唯一），`source_trace` 保留全部来源审计
- **事件类型**：第一次/第二次/第N次审查意见通知书、驳回决定、授权通知、补正通知、其他官方通知——授权/驳回只建提醒记录，**不改变专利主档的法律状态**
- **保护规则不变**：HIGH 置信度才自动新增；已有记录缺官文日只补日期；日期不一致标 `date_conflict` 待人工确认；人工录入的处理人/撰写人/摘要/期限永不被覆盖
- **CNIPA 登录由用户完成**：前两层失败且 CNIPA 未登录时只提示"需要登录"，绝不自动弹浏览器；菜单 `CNIPA 登录` 打开可见浏览器人工登录（不读密码、不自动验证码），登录态持久化复用
- EPO OPS Key 只用于同族查询/法律状态功能，与巡检数据源无关；USPTO/EPO 公共案卷不使用任何 API Key
- 详细红线与调试方法见 `tools/web_dossier/README.md`
